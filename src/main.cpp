...
void readParameters(char *parFile) {
 ...
 while (fin.good()) {
  ...
  /*else if (strcmp(parName, "s0ScaleFactor") == 0)
   ...
   targZ = atoi(parValue);
  else if (strcmp(parName, "xi_fa") ==0)
   xi_fa = atof(parValue);
  else if (strcmp(parName, "xi_q") ==0)
   xi_q = atof(parValue);
  else if (strcmp(parName, "xi_h") ==0)
   xi_h = atof(parValue);
  else if (strcmp(parName, "alpha") ==0)
   alphaxs = atof(parValue);
...
void printParameters() {
 ...
 cout << "xi_q = " << xi_q << endl;
 cout << "xi_h = " << xi_h << endl;
 cout << "alpha = " << alphaxs << endl;
 cout << "beta = " << betaxs << endl;
...
int main(int argc, char **argv) {
 ...
 } else if (eosTypeHadron == 2) {
   eosH = new EoShadronPH(); //hadronic EoS generated by Pasi Huovinen, in 2025
 } else {
